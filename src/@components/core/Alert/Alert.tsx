      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1),0_10px_10px_-5px_rgba(0,0,0,0.04)] overflow-hidden transform transition-all duration-200 scale-100 opacity-100">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex justify-end space-x-3">
            <Button
              onClick={onCancel}
              className="px-5 py-2 cursor-pointer text-sm font-medium !text-black bg-white border border-gray-200 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-200 transition-all duration-150 shadow-sm"
            >
              {cancelLabel}
            </Button>
            <Button
        ...
        {isLoading && (
          <div className="absolute top-0 left-0 right-0 h-1 bg-primary-lighter overflow-hidden">
            <div className="absolute top-0 left-0 bottom-0 w-1/2 bg-primary-muted animate-loading-bar"></div>
          </div>
        )}
      </div>
    </div>

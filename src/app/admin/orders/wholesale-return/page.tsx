...
const Page: React.FC = () => {
  ...
  const handleAddClick = () => {
    router.push("/admin/orders/wholesale-return/create-purchases");
  };
  ...
  const fetchPurchasesReturn = () => {
    ...
    WholesaleReturnService.getWholesaleReturn({
  ...
  }, [canFetchPageData, debouncedSearchTerm, currentPage, productPerPage]);
  ...
  const confirmRemove = async () => {
    if (!remove) return;
    try {
      const res = await WholesaleReturnService.deleteWholesaleReturn(remove);
      if (res?.success) {

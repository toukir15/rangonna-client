...
import {
  ICityOrderReportItem,
  ICityOrderReportResponse,
...
const Page: React.FC = () => {
  const [tableData, setTableData] = useState<ICityOrderReportItem[]>([]);
  ...
  const fetchCampaignReportGoogle = async () => {
    ...
    CampaignReportService.getCitiesReport({
      page: currentPage,
      limit: productPerPage,
      startDate: formattedFrom,
      endDate: formattedTo,
    })
      .then((res: ICityOrderReportResponse) => {
        if (res?.success) {
      ...
      <div className="2xl:px-4 px-3 2xl:pt-4 md:pt-3 pt-2 pb-4 relative w-full">
        ...
        <div className="data-table-card glass-card rounded-2xl orders-table-shell">
          <div className="premium-table-toolbar">
            <p className="premium-table-toolbar-title">Cities records</p>
        ...
        >
          ...
          <Tbody>
            {tableData?.map((data: ICityOrderReportItem, index: number) => {
              const total = data?.total_count || 0;

              return (
                <Tr key={index}
                >
                  <Td><span className="data-table-primary">{data?.city_name}</span></Td>
                  <Td><span className="table-amount">{total}</span></Td>
                  <Td>
                    {data?.transit}{" "}
                    <span className="text-xs text-gray-500">
                      ({getPercent(data?.transit, total)})
                    </span>
                  </Td>
                  ...
                  <Td>
                    {data?.delivery}{" "}
                    <span className="text-xs text-gray-500">
                      ({getPercent(data?.delivery, total)})
                    </span>
                  </Td>

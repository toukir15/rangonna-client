...
import {
  IUtmCampaignReportItem,
  IUtmCampaignReportResponse,
...
const Page: React.FC = () => {
  const [tableData, setTableData] = useState<IUtmCampaignReportItem[]>([]);
  ...
  const [range, setRange] = useLocalStorageDateRange(
    "facebookCampaignSetUtmTermReportDateRange",
    DEFAULT_DATE_RANGE
  );
  ...
  const fetchCampaignReportFacebookAds = async () => {
    ...
    CampaignReportService.getCampaignReportFb({
      page: currentPage,
      limit: productPerPage,
      startDate: formattedFrom,
      endDate: formattedTo,
    })
      .then((res: IUtmCampaignReportResponse) => {
        if (res?.success) {
  ...
  return (
    <AuthLayout>
      <div className="2xl:px-4 px-3 2xl:pt-4 md:pt-3 pt-2 pb-4 relative w-full">
        <PageHeader title="Facebook Campaign Report" />
        
        <div className="data-table-card glass-card rounded-2xl orders-table-shell">
          <div className="premium-table-toolbar">
            <p className="premium-table-toolbar-title">Facebook Campaign records</p>
          ...
          <div className="data-table-toolbar">
            ...
            <div className="data-table-toolbar-end">
              <TableRefreshButton
                onRefresh={fetchCampaignReportFacebookAds}
        ...
        >
          ...
          <Tbody>
            {tableData?.map((data: IUtmCampaignReportItem, index: number) => {
              const total = data?.total_order || 0;

              return (
                <Tr key={index}
                >
                  <Td><span className="data-table-primary">{data?.utm_campaign_id}</span></Td>

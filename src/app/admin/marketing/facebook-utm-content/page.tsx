...
import {
  IFacebookUtmContentReport,
  IFacebookUtmContentReportResponse,
...
const Page: React.FC = () => {
  const [tableData, setTableData] = useState<IFacebookUtmContentReport[]>([]);
  ...
  const [range, setRange] = useLocalStorageDateRange(
    "facebookAdsUtmContentReportDateRange",
    DEFAULT_DATE_RANGE
  );
  ...
  const fetchCampaignReportFacebookAds = async () => {
    ...
    CampaignReportService.getCampaignReportFbAds({
      page: currentPage,
      limit: productPerPage,
      startDate: formattedFrom,
      endDate: formattedTo,
    })
      .then((res: IFacebookUtmContentReportResponse) => {
        if (res?.success) {
  ...
  return (
    <AuthLayout>
      <div className="2xl:px-4 px-3 2xl:pt-4 md:pt-3 pt-2 pb-4 relative w-full">
        <PageHeader title="Facebook Ads Report" />
        
        <div className="data-table-card glass-card rounded-2xl orders-table-shell">
          <div className="premium-table-toolbar">
            <p className="premium-table-toolbar-title">Facebook Ads records</p>
        ...
        >
          ...
          <Tbody>
            {tableData?.map(
              (data: IFacebookUtmContentReport, index: number) => {
                const total = data?.total_order || 0;

                return (
                  <Tr key={index}
                  >
                    <Td><span className="data-table-primary">{data?.utm_content_id}</span></Td>

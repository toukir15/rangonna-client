...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    if (pId && singleData) {
      ...
      setOrderDetails({
        purchase_products: singleData.purchase_products.map((item: any) => {
          return {
      ...
      singleData.purchase_products.forEach((item: any) => {
  ...
  const formSubmit = (data: PurchaseFormData) => {
    // if (!orderDetails?.unit_cost || 0) {
    //   ToastService.error("purchase price is missing.");
    //   return;
    // }

    const missingCostItem = orderDetails.purchase_products.find(
      (item: { unit_cost: null | undefined }) =>
    ...
    if (pId) {
      setIsSubmit(true);
      PurchasesService.updatePurchases(pId, {
        ...data,
        ...
        .then((res: any) => {
          if (res?.success) {
            ...
            router.push(`/admin/purchase/purchase/view/${pId}`);
          } else {
  ...
  const handleProductSelect = (product: any) => {
    ...
    setOrderDetails((prev: { purchase_products: any[] } | null) => ({
      purchase_products: [
  ...
  const decrementQuantity = (index: number) => {
    if (!orderDetails || !orderDetails.purchase_products) return;
    ...
    if (currentQuantity > 1) {
      ...
      setOrderDetails({ purchase_products: newItems });
    }
  };
  ...
  const calculateSubtotal = () => {
    ...
    return orderDetails?.purchase_products.reduce(
  ...
  const handleQuantityChange = (index: number, value: string) => {
    ...
    // invalid number হলে কিছু করবে না
  ...
  const hasNegativeValue = shipping < 0 || discount < 0;
  return (
    <AuthLayout>
      <NoScrollLayout>
      ...
      <form onSubmit={handleSubmit(formSubmit)} className=" px-4 min-h-[75vh]">
                            ...
                            <td className="border dark:border-gray-600 dark:text-gray-300 px-4 py-2">
                              <div className="flex items-center justify-center">
                                ...
                                <input
                                  ...
                                  className="w-20 text-center py-1 border-t border-b dark:border-gray-500 dark:bg-gray-700"
                                  min={1}
                                />
        ...
        <div className="flex gap-3 items-end justify-end">
          ...
          <Button
            type="submit"
            className={`rounded-m flex justify-center font-medium text-white ${isSubmit || hasNegativeValue
              ? "bg-gray-400 cursor-not-allowed"
              : "bg-blue-500"
              }`}
            disabled={isSubmit || hasNegativeValue || isButtonDisable}
          >

...
const Page: React.FC = () => {
  ...
  const handleAddClick = () => {
    router.push("/admin/purchase/purchase/create-purchases");
  };
  ...
  const confirmRemove = async () => {
    if (!remove) return;
    try {
      const res = await PurchasesService.deletePurchases(remove);
      if (res?.success) {
              ...
              {purchasesData?.map((item: any, index: number) => {
                return (
                  <Tr className="h-14" key={index}>
                    ...
                    <Td>
                      <Link
                        href={`/purchase/purchase/view/${item?._id}`}
                            ...
                            {permissionList.includes("purchase_edit") && (
                              <>
                                <button
                                  ...
                                  onClick={() =>
                                    router.push(
                                      `/purchase/purchase/edit-purchases/${item?._id}`
                                    )
                                  }
                                >
                                  Edit

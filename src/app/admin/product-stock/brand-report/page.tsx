  ...
  return (
    <AuthLayout>
      <div className="2xl:px-4 px-3 2xl:pt-4 md:pt-3 pt-2 pb-4 relative w-full">
        <PageHeader title="Warehouse Brand Report" />
        
        <div className="data-table-card glass-card rounded-2xl orders-table-shell">
          <div className="premium-table-toolbar">
            <p className="premium-table-toolbar-title">Warehouse Brand records</p>
          ...
          <Tbody>
            {warehouseBrandData?.map((brandData: any, index: number) => {
              return (
                <Tr key={index}
                >
                  <Td><span className="data-table-primary">{brandData?.brand}</span></Td>

                  <Td><span className="table-amount">{brandData?.remaining_stock - brandData?.transit_quantity}</span></Td>
                </Tr>
              );
            })}
          </Tbody>

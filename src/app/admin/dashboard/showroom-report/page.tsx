...
import DashboardQuickReportTableAdmin from "@admin/components/pages/DashBoard/DashboardQuickReportTableAdmin";
import DashBoardShowroomExpenseTableAdmin from "@admin/components/pages/DashBoard/DashBoardShowroomExpenseTableAdmin";
import DashBoardShowroomTableAdmin from "@admin/components/pages/DashBoard/DashBoardShowroomTableAdmin";
import ShowroomQuickViewModalAdmin from "@admin/components/pages/DashBoard/ShowroomQuickViewModalAdmin";
import { noPermission } from "@admin/utils/constant";
import AllFilter from "@admin/components/pages/AllFilter/AllFilter";
import Button from "@admin/components/core/Button/Button";
import Icon from "@admin/components/core/Icon/Icon";

export const DashboardShowroomReportContext = createContext({} as any);
...
const Page: React.FC = () => {
    ...
    const [selectedWarehouse, setSelectedWarehouse] = useState<SelectOption>({
        value: "all",
        label: "All Warehouse",
    });
    ...
    const fetchReturnList = () => {
        ...
        DashboardShowroomService.getAdminDashboardShowroomList({
    ...
    const fetchExpensesReportList = () => {
        ...
        DashboardShowroomService.getAdminExpenseReport({
            startDate: formattedFrom,
            endDate: formattedTo,
    ...
    const fetchListQuick = () => {
        ...
        DashboardShowroomService.getAdminShowroomListQuick({
    ...
    const fetchCartData = () => {
        ...
        DashboardShowroomService.getAdminCartList({
            startDate: formattedFrom,
            endDate: formattedTo,
            warehouse: selectedWarehouse.value
        })
    ...
    useEffect(() => {
        ...
        setQuickCurrentPage(1);
    }, [range, selectedWarehouse]);

    useEffect(() => {
    if (!canFetchPageData) return;
    fetchReturnList();
  }, [canFetchPageData, range, selectedWarehouse]);

    useEffect(() => {
    if (!canFetchPageData) return;
    fetchExpensesReportList();
  }, [canFetchPageData, range, expenseOrdersPerPage, expenseCurrentPage, selectedWarehouse]);

    useEffect(() => {
    if (!canFetchPageData) return;
    fetchListQuick();
  }, [canFetchPageData, debouncedSearchTerm, range, quickOrdersPerPage, quickCurrentPage, selectedWarehouse]);
    ...
    return (
        <AuthLayout>
            <NoScrollLayout>
...

                {
                    isFilterOpen && <div className="md: mx-4">
                        <AllFilter
                            isWarehouseFilter={true}
                            isFilterOpen={isFilterOpen}
                            warehouseDataOption={warehouseDataOption}
            ...
            <div>
                {
                    hasPermission(permissionList, "showroom_view_admin") &&
            ...
            <div className="min-h-[75vh] 2xl:px-4 px-3">
                <div className="xl:mt-3 mt-2">
                    <DashboardShowroomReportContext.Provider
                        value={{
                            returnListData,
                            tableLoading,
                            setIsModalOpen,
                    ...
                    >
                        {hasPermission(permissionList, "showroom_payment_history_admin_view") && <DashBoardShowroomTableAdmin />}

                        {hasPermission(permissionList, "showroom_expense_report_view_admin") &&
                            expenseListData?.length > 0 && (
                                <div className="mt-4 bg-white rounded-lg p-2">
                                    <DashBoardShowroomExpenseTableAdmin />
                                    <PaginationComponent
                        ...
                        {hasPermission(
                            permissionList,
                            "showroom_payment_history_admin_quick_view"
                        ) &&
                            expenseQuickData.length > 0 && (
                                <div className="mt-4 bg-white rounded-lg p-2">
                                    <DashboardQuickReportTableAdmin />

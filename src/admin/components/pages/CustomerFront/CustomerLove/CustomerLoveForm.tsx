...
type CustomerLoveFormProps = {
    mode: "create" | "edit";
    initialData?: TCustomerLoveData | null;
    reviewId?: any;
};
...
const schema: yup.ObjectSchema<FormValues> = yup.object({
    ...
    mobile: yup
        .array()
        .of(itemSchema)
        .min(1, "At least one mobile review is required")
        .required(),
    desktop: yup
        .array()
        .of(itemSchema)
        .min(1, "At least one desktop review is required")
        .required(),
});

const uploadImage = async (file: File) => {
    const data = await CustomerLoveService.uploadFileDirect(file, "customer-love");
                        ...
                        >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input
                                        {...register(`${type}.${index}.city`)}
                                        placeholder="Dhaka"
                                        className="w-full border rounded-lg px-3 py-2 bg-white dark:bg-gray-900 dark:border-gray-600 dark:text-white"
                                    />
                                    {itemError?.city?.message && (
                                        <p className="text-red-500 text-xs mt-1">
                                            {itemError.city.message}
                                        </p>
                                    )}
                                ...
                                <div>
                                    ...
                                    <input
                                        {...register(`${type}.${index}.product`)}
                                        placeholder="Bridal Ivory Heritage"
                                        className="w-full border rounded-lg px-3 py-2 bg-white dark:bg-gray-900 dark:border-gray-600 dark:text-white"
                                    />
                                    {itemError?.product?.message && (
                                        <p className="text-red-500 text-xs mt-1">
                                            {itemError.product.message}
                                        </p>
                                    )}
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        min={1}
                                        max={5}
                                        {...register(`${type}.${index}.rating`)}
                                        placeholder="5"
                                        className="w-full border rounded-lg px-3 py-2 bg-white dark:bg-gray-900 dark:border-gray-600 dark:text-white"
                                    />
                                    {itemError?.rating?.message && (
                                        <p className="text-red-500 text-xs mt-1">
                                            {itemError.rating.message}
                                        </p>
                                    )}
                                ...
                                <div className="md:col-span-2">
                                    ...
                                    <textarea
                                        {...register(`${type}.${index}.text`)}
                                        placeholder="The bridal set looked even more beautiful in person."
                                        rows={3}
                                        className="w-full border rounded-lg px-3 py-2 bg-white dark:bg-gray-900 dark:border-gray-600 dark:text-white"
                                    />
                                    {itemError?.text?.message && (
                                        <p className="text-red-500 text-xs mt-1">
                                            {itemError.text.message}
                                        </p>
                                    )}
...
const CustomerLoveForm: React.FC<CustomerLoveFormProps> = ({
    mode,
    initialData = null,
    reviewId,
}) => {
    ...
    const formSubmit = async (data: FormValues) => {
        ...
        try {
            const res =
                mode === "edit" && reviewId
                    ? await CustomerLoveService.updateCustomerLove(reviewId, payload)
                    : await CustomerLoveService.createCustomerLove(payload);

            if (res?.success) {
                ToastService.success(res?.message || "Customer review saved successfully");
                router.push("/admin/customer-front/customer-review");
            } else {
                ToastService.error(res?.message || "Failed to save customer review");
            }
        } catch (err: any) {
                ...
                <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div className="flex items-center justify-between border-b dark:border-gray-700 px-4 md:px-6 py-4">
                        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
                            {mode === "edit" ? "Edit Customer Review" : "Create Customer Review"}
                        </h3>
...
export default CustomerLoveForm;

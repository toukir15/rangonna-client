...
import OrderHistory from "../../Orders/ViewOrder/OrderHistory";
...
const WholeSaleOrderNote: React.FC<OrderNotesProps> = ({
  orderId,
  showCustomerNote,
  fetchOrdersDetail,
  isLoading,
  ...
  const handleSubmitCustomerNote = async (e: React.FormEvent) => {
    ...
    try {
      const res = await wholesaleOrderService.updateCustomerNote(orderId, {
  ...
  const handleSubmitOrderNote = async (e: React.FormEvent) => {
    ...
    wholesaleOrderService
      .noteUpdate(orderId, { text: orderNoteText })
      .then((res: any) => {
        if (res?.success) {
          ToastService.success(res?.message);
          fetchOrdersDetail();
...
export default WholeSaleOrderNote;

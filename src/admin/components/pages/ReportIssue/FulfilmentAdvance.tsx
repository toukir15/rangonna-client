...
const FulfilmentAdvance = ({
    ...
    const { orderList } = useContext(fulfillmentContext);
    const [selectedPaymentId, setSelectedPaymentId] = useState<string | null>(null);
    const [currentMode, setCurrentMode] = useState<"Add" | "Edit" | "View">(modalMode);
    const currentOrder = orderList?.find((order: any) => order?._id === orderId);
    const dueAmount = currentOrder?.due ? String(currentOrder.due) : "";
    ...
    useEffect(() => {
        ...
        if (currentMode === "Edit" && selectedPaymentId) {
            const selectedAdvance = advanceData?.find(
                (a: any) => a?._id === selectedPaymentId
            );

            const selectedPayment = paymentMethodOptions.find(
                (p: any) => p.value === selectedAdvance?.payment_method
            );

            reset({
                amount: selectedAdvance?.amount ? String(selectedAdvance.amount) : "",
    ...
    const formSubmit = async (formData: any) => {
        ...
        if (currentMode === "Edit") {
            const updateId = selectedPaymentId || orderId;

            AdvanceSalaryService.updateAdvanceOrder(updateId, payload)
                .then((res: any) => {
                    if (res?.success) {
            ...
            AdvanceSalaryService.createAdvanceOrder(payload)
                .then((res: any) => {
                    if (res?.success) {
                        ToastService.success(res?.message);
                        closeModal();
...
export default FulfilmentAdvance;